...
void optimize(PFSymbolTable *pContext)
{
   ...
   FFTEntryNew *_fftMatrix = fftMatrixNew;
	if (CPU_FLAGS & CPU_SSE2)
	{
		...
		PFPrintfLog ("Running with the SSE2 context sizes (Pentium 4 detected)\n");
	}

	for(x=0;_fftMatrix[x].dwElemCnt;x++) {}
	int xcount=x;
	PFPrintfLog("%d different FFT sizes installed\n\n",xcount);

	char Buffer[512];
	getCpuDescription(Buffer, 1);
	PFPrintfLog("CPU Information (From Woltman v25 library code)\n%s\n\n", Buffer);
	...
	for(x=0;x<xcount;x++)
	{
		...
		for(DWORD B=BB-1;B<=BB+1;B++)
		{
			...
			PFPrintfLog("Testing (5^%d-1)/2^%d for FFT length %lu*%lu\n",iPow5,idiv,C,B);
			{
				GWInteger gwN;
				Integer NN;
				...
				{
					PFPrintfLog("Test OK.\nTimings:\n");
					PFPrintfLog("eval:%0.8f\nflattogw:%0.8f\ngwtoflat:%0.8f\n",deval,dftogw,dgwtof);
				}
				else
				{
					PFPrintfLog("Conversion test failed!!!!\n");
					PFPrintfLog("RR=");
			...
			// will deallocate GWIntegers
			{
				...
				d1 = Timer.GetSecs();
				PFPrintfLog("Iteration: %0.8fs\n",d1/dwIterations);

				double ex=sumX/sumN;
				double exx=sumXX/sumN;
				double s2=exx-ex*ex;
				double s=sqrt(s2);
				PFPrintfLog("Roundoff: mean:%f s.d.:%f confidence:%f\n\n", ex,s,(0.5-ex)/s);
