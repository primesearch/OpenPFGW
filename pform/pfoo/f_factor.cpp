...
void F_Factor::OnPrompt()
{
   ...
   if (g_nIterationCnt && Timer.GetSecs() > 5)
   {
      char Buf[256];
      if (*g_cpTestString)
         snprintf(Buf, sizeof(Buf), "F: %.50s %u/%u (trial factor to %" PRIu64")\r",g_cpTestString, m_dwStepsDone,m_dwStepsTotal,pmax);
      else
         snprintf(Buf, sizeof(Buf), "F: %u/%u (trial factor to %" PRIu64")\r",m_dwStepsDone,m_dwStepsTotal,pmax);
